...
exports.generate = function (req, res) {
    ...
    global.getConfig().then(function (config) {
        feed = new Rss({
            ...
            feed_url: '{0}/rss/movies'.format(config.publicHost),
        ...
        Movie.find().sort('-createdAt').limit(20).exec().then(function (movies) {
            ...
            for (i = 0; i < movies.length; i++) {
                ...
                if (movie.needsReview) {
                    ...
                    description += '<p><a href="{0}">Mark as interesting</a></p>'.format(markAsInterestingUrl.format(config.publicHost, movie.id));
                }
                feed.item({
                    title:  movie.title,
